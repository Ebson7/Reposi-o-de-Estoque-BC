...
${isValidadeCurta ? '⚠️ *ATENÇÃO:* Validade Curta\n' : ''}${observacoes ? `📝 *Obs:* ${observacoes}\n` : ''}🏢 *Estoque Marsil:* ${selectedProduct.estoqueMarsil} | *Boracéia:* ${selectedProduct.estoqueBoraceia}`;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Vendor Selection & Active Order Status Bar */}
      <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-3.5 flex-1 min-w-0">
          <div className="w-11 h-11 rounded-xl bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-900/60 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1 flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 truncate">
                {solicitante 
                  ? `Pedidos registrados para: ${solicitante}`
                  : 'Selecione seu usuário para fazer pedidos'}
              </span>
            </div>
          </div>
        </div>
